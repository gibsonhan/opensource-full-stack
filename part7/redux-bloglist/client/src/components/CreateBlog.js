...
const CreateBlog = props => {
  ...
  const handleCreate = async e => {
    ...
    try {
      ...
      props.sendMessage(
        `New Blog: ${newBlog.title} by ${newBlog.author} was created`,
        'green',
        7
      );
    } catch (exception) {
    ...
    <div>
      ...
      <form onSubmit={handleCreate}>
        <div>
          {' '}
          title
          <input data-cy="title" {...title.input()} />
        </div>
        <div>
          author
          <input data-cy="author" {...author.input()} />
        </div>
        <div>
          url
          <input data-cy="url" {...url.input()} />
        </div>
        <button data-cy="createBlog">Create</button>
      </form>
    </div>
  );
};
